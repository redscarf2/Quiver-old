...
static int GetAttlistDecl(LPXMLPARSER parser, LPXMLVECTOR *newAtts);
static int DestroyDeclAttTableProc(char *key, void *data, void *userdata);
static int ParseElementDecl(LPXMLPARSER parser);
static int ParseChildren(LPXMLPARSER parser);
static int ParseCp(LPXMLPARSER parser);
static int ParseMixed(LPXMLPARSER parser);
...
#include "optcfg.h"
#else
#define ReadCh_Opt ReadCh
#define XMLStringbuf_Append_Opt XMLStringbuf_Append
#endif
...
{	
	...
	if (*c == '#') {
		...
		else if (!(XMLStringbuf_Append(sbuf, ref, i)))
			return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
	}
	else {
				...
				if (sbuf->len) {
					if (HANDLER(characters)) {
						if (HANDLER(characters)(parser->UserData, sbuf->str, sbuf->len)
							== XML_ABORT) return Er_(parser, ERR_XMLP_ABORT);
					}
...
static int ParseContent(LPXMLPARSER parser)
{
	...
	if (iLT) {
		if (EINPUT(BufferedIStream_ResetBuf(PREADER, iLF))) return 0;
		PREADER->pos+=iLT;
	}
	return (c != (XMLCH*)NULL);
...
static int ParseStartTag(LPXMLPARSER parser)
{
	...
	if (!parser->DocumentElement) {
		if (RT->doctypeName) {
			if (strcmp(pTag->qname, RT->doctypeName))
				return Er_(parser, ERR_XMLP_EXPECTED_FOUND, RT->doctypeName, pTag->qname);
			parser->DocumentElement = RT->doctypeName;
		}
		else if (!((parser->DocumentElement = xmlMemdup(pTag->qname, namelen+1))))
			return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
	}
	...
	if (!endChars || 
		(RT->declAttTable && 
		(DeclAtts = XMLHTable_Lookup(RT->declAttTable, pTag->qname)))) {
			if (!(ParseAttributes(parser, pTag, &isEmpty, DeclAtts))) return 0;
	}
						   ...
						   int *isEmpty, LPXMLVECTOR DeclAtts)
{
	...
	int processNS = _XMLParser_GetFlag(parser, XMLFLAG_NAMESPACES);
	...
	{	
		if (!DeclAtts) {
			...
			for (pAtt=NULL; iDeclAtts < DeclAtts->length;) {
				struct tagDECLATT *pDeclAtt = XMLVector_Get(DeclAtts, iDeclAtts++);
...
static int ParsePI(LPXMLPARSER parser, int skip)
{
	...
	if (namelen == 3 && !stricmp(target, TOK_XML))
		return Er_(parser, ERR_XMLP_INVALID_DECL, TOK_XML);
...
{	
	...
	p->XMLFlags = XMLFLAG_NAMESPACES | XMLFLAG_CONVERT_EOL |
...
{	
	...
	if (parser->DocumentElement || RT->doctypeName)
	...
	if (!(RT->doctypeName = xmlMemdup(PREADER->buf+l, namelen+1)))
		return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
...
{	
	...
	while (ParseContentDTD(parser, isExternal)) {
		...
		if (t == 1) {
			...
			if (!GetAttlistDecl(parser, &newAtts)) {
				if (newAtts) DestroyDeclAttTableProc(NULL, newAtts, NULL);
				if (!parser->ErrorCode)
					Er_(parser, ERR_XMLP_INVALID_DECL, "ATTLIST");
				break;
			}
		}
		else if (t == 7) {
...
*/
static int ParseEnumeration(LPXMLPARSER parser)
{
	int len;
...
{	
	...
	switch(TrieTok(parser, TRelTypes, 0)) {
		...
		case 3:
			...
			if (!XMLStringbuf_AppendCh(&RT->charsBuf, '('))
				return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
			switch(TryTok(parser, "#PCDATA", 0)) {
				case -1:
					if (!ParseChildren(parser)) return 0;
					break;
				case 0:
					if (!XMLStringbuf_Append(&RT->charsBuf, "#PCDATA", 7))
						return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
					if (!ParseMixed(parser)) return 0;
					break;
	...
	if (HANDLER(elementDecl)) {
		...
		if (HANDLER(elementDecl)(parser->UserData, 
			RT->dtd->sbuf.str, (XMLCH*) RT->charsBuf.str)==XML_ABORT)
				return Er_(parser, ERR_XMLP_ABORT);
	}
...
{	
	...
	if(CURCHAR==')') {
		...
		if (CURCHAR=='?'||CURCHAR=='*'||CURCHAR=='+') {
			if (!XMLStringbuf_AppendCh(&RT->charsBuf, CURCHAR))
				return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
			IPOS(1);
		}
		return 1;
		...
		sep[0] = CURCHAR;
		if (!XMLStringbuf_AppendCh(&RT->charsBuf, sep[0]))
			return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
		IPOS(1);
	}
...
static int ParseCp(LPXMLPARSER parser)
{
	if (CURCHAR=='(') {
		...
		int l;
		if (ParseNameDTD(parser, &l, NULL, &RT->charsBuf, ")|,?*+", 6) == -1) 
			return 0;
		RT->charsBuf.len--;
		if (CURCHAR=='?'||CURCHAR=='*'||CURCHAR=='+') {
			if (!XMLStringbuf_AppendCh(&RT->charsBuf, CURCHAR))
...
{	
	...
	while (CURCHAR!=')') {
		...
		if (!XMLStringbuf_AppendCh(&RT->charsBuf, '|'))
			return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
		if (SkipWS(parser) == -1) return 0;
		if (ParseNameDTD(parser, &n, NULL, &RT->charsBuf, ")|", 2) == -1) 
			return 0;
		RT->charsBuf.len--;
		if (SkipWS(parser) == -1) return 0;
	}
	...
	if (!XMLStringbuf_Append(&RT->charsBuf, ")*", 2))
		return Er_(parser, ERR_XMLP_MEMORY_ALLOC);
	return 1;
}
	...
	*newAtts = NULL; /* if there will be newAtts hanging around
	...
	while (CURCHAR!='>') {
		...
		if (CURCHAR == '(') {
			...
			IPOS(1);
			if (!ParseEnumeration(parser)) return 0;
			if (!REQUIRE_WS(i)) return 0;
		}
		else {
		...
		if (dAtt.value || dAtt.type != XMLATTDECL_TYPE_CDATA) {
			...
			if (!oldAtts && !*newAtts && 
				!(oldAtts = XMLVector_Create(newAtts, 2, sizeof(struct tagDECLATT))))
					return Er_(parser, ERR_XMLP_MEMORY_ALLOC);			
			...
			if (!XMLVector_Append(oldAtts, &dAtt))
				return Er_(parser, ERR_XMLP_MEMORY_ALLOC);		
		}
		...
		if (dAtt.type == XMLATTDECL_TYPE_ENUMERATED ||
			dAtt.type == XMLATTDECL_TYPE_NOTATION)
			XMLStringbuf_SetLength(&RT->charsBuf, 0);
	}
...
static int DestroyDeclAttTableProc(char *key, void *data, void *userdata)
{	
	struct tagDECLATT *a, *pEnd;
	pEnd = (struct tagDECLATT*)_XMLVector_GetIterP((LPXMLVECTOR)data, a);
	for (; a!=pEnd; a++) {
		free(a->name);
		if (a->value) free(a->value);
	}
	XMLVector_Free((LPXMLVECTOR)data);
