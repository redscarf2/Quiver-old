...
REM ****************
rem "C:\Program Files (x86)\Microsoft Visual Studio 8\VC\bin\nmake.exe" /C -f makefile.%inputbase% clean > clean.txt 2>&1
...
if exist "filelist.txt" if exist "uniquefilestocopy.txt" if not "%dynamic_shaders%" == "1" (
	...
	cd %ChangeToDir%
	%shadercompilecommand% -nompi -mpi_workercount %shadercompileworkers% -allowdebug -shaderpath "%shader_path_cd:/=\%" %x360_args% %SDKArgs%
	cd %shader_path_cd%
)
